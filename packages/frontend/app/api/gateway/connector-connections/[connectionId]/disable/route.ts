...
type ConnectorConnectionDisableRouteContext = {
  params: Promise<{
    connectionId: string;
  }>;
};
...
export async function POST(
  request: NextRequest,
  context: ConnectorConnectionDisableRouteContext
): Promise<NextResponse> {
  const { connectionId } = await context.params;
  try {
    const upstream = await fetch(
      gatewayUrl(`/connector-connections/${encodeURIComponent(connectionId)}/disable`),
      {
        method: "POST",
        headers: {
          ...
          "X-Synarch-Trace-Id":
            ...
            `trace_frontend_connector_disable_${Date.now()}`
        },
