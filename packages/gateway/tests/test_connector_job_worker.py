...
def test_run_loop_continues_after_transient_error(
    ...
    monkeypatch.setattr(connector_job_worker, "record_worker_heartbeat", fake_heartbeat)
    ...
    assert recorded_heartbeats[1]["worker_kind"] == "connector_job"
    assert recorded_heartbeats[1]["target"] == "cron"
