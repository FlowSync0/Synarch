...
| --- | --- | --- | --- | --- |
| A. Interface | Partial | Next.js control surface plus `/app` operator workspace with live goal creation, project list, project brief, operator actions, readiness, and connector connection setup. The detailed dashboard still exposes agents, lifecycle approvals, timeline events, review queue, connector job controls, and connector history. | No live cost dashboard yet. | Create goal from `/app`, run ready tasks, connect a provider, then inspect timeline/audit trace. |
| B. Orchestration | Partial | Gateway accepts `GoalEnvelope`, persists goals through `/goals/submit`, runs one ready task through `/tasks/run-next`, recovers expired leases before bounded `/tasks/run-ready` batches, respects retry backoff, records scheduler ticks, skips task claim conflicts, exposes an opt-in scheduler worker, and state-service has a generic durable `work_queue_items` table for non-task workers. | Routing is keyword-based only and the generic work queue is not yet wired into every worker loop. | Submit goal -> run scheduler tick -> persisted result timeline. |
...
| E. Project / Workflow | Partial | Project/task contracts, durable repositories, task dependencies, atomic task start claim, task lease heartbeat, expired lease retry recovery, retry backoff, dead-letter review metadata, task review decisions, task result recording, bounded ready-batch execution, scheduler tick event/audit records, opt-in scheduler worker, generic durable work queue claim/complete/fail/recover endpoints, and timeline events exist. | No dedicated UI for generic work-queue items yet. | Bounded scheduler loop executes only ready tasks and emits traceable batch output. |
...
| G. Execution & Tooling | Partial | `ToolCallRequest`, `ToolResult`, gateway permission gate, `event.emit`/`web.fetch`/`web.extract` adapters, `local_fetch`, `local_playwright`, Firecrawl, and Browserless provider selection, task-scoped credential gates, service health checks, SecretVault-backed connector connections, durable connector job lifecycle records, and bounded connector workers exist. | No sandbox execution and no OAuth account connector flow yet. | Connector job executes a real adapter only after permission, service, credential, and SecretVault reference gates pass. |
...
- Done: state-service exposes a generic durable `work_queue_items` queue with create/list,
  ...
  migration, and audit logs for non-task workers such as reminders, ingestion, webhooks, and
  maintenance.
