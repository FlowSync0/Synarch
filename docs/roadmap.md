...
| F. Memory & Context | Partial | PostgreSQL-backed memory items, scoped context assembly, deterministic ranking, token budget enforcement, proposed memory candidates, gateway approval/rejection, dashboard review controls, live approved-memory reuse validation, and live rejected-memory exclusion validation exist. | No compaction, vector search, graph retrieval, or hierarchical context database. | Add memory compaction for long-running projects without losing provenance. |
