...
| B. Orchestration | Partial | Gateway accepts `GoalEnvelope`, persists goals through `/goals/submit`, runs one ready task through `/tasks/run-next`, runs bounded project batches through `/tasks/run-ready`, and exposes a scheduler tick script. | Routing is keyword-based only and there is no persisted scheduler lease/heartbeat yet. | Submit goal -> run scheduler tick -> persisted result timeline. |
...
| E. Project / Workflow | Partial | Project/task contracts, durable repositories, task dependencies, task result recording, bounded ready-batch execution, scheduler tick script, and timeline events exist. | No durable worker queue, scheduler lease, or retry policy yet. | Bounded scheduler loop executes only ready tasks and emits traceable batch output. |
