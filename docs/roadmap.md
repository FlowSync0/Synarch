...
| B. Orchestration | Partial | Gateway accepts `GoalEnvelope`, persists goals through `/goals/submit`, runs one ready task through `/tasks/run-next`, and runs bounded project batches through `/tasks/run-ready`. | Routing is keyword-based only and there is no persistent scheduler process yet. | Submit goal -> run bounded ready batch -> persisted result timeline. |
...
| E. Project / Workflow | Partial | Project/task contracts, durable repositories, task dependencies, task result recording, bounded ready-batch execution, and timeline events exist. | No continuous scheduler loop or worker queue yet. | Bounded batch run executes only ready tasks and stops with a clear reason. |
