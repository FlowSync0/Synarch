...
GoalEnvelope
  ...
  -> `/app` reads Gateway cost records and summaries through same-origin proxies for live AI spend visibility without exposing provider secrets
...
`noop`, `log`, and `project.reminder.emit`; unknown actions are failed through the durable queue path
and dead-lettered instead of being executed speculatively. `project.reminder.emit` validates
`project_id` and `message`, writes a durable `project.reminder` event, then completes the queue item
with the emitted event id.
...
- `SECRET_VAULT_REQUIRE_ENCRYPTION=true` makes Gateway readiness block and makes connector secret
  storage fail before state mutation unless `SECRET_VAULT_KEY` or `SECRET_VAULT_KEY_FILE` enables
  encryption and legacy plaintext entries have been re-encrypted.
